...
#-----------------------------------------------------------------------------------
# Load new data with manual weather features
#-----------------------------------------------------------------------------------
parquet_file = 'data/MexMigData_manual_features.parquet'
...
#-----------------------------------------------------------------------------------
df = df.sort_values(by=['numc','year'])
...
vill_dummies = pd.get_dummies(df['villageid'], drop_first=True,
                             prefix="vill", dtype=int)
...
df = df[cols]
df.to_csv('data/data_cohort_daily_weather.csv', index=False)
print("Cohort prep for 1980–2007 with manual weather features complete. Rows:", df.shape[0])
